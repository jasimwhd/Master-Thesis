%!TEX root = ./main.tex
%
% This file is part of the i10 thesis template developed and used by the
% Media Computing Group at RWTH Aachen University.
% The current version of this template can be obtained at
% <http://www.media.informatik.rwth-aachen.de/karrer.html>.

\documentclass[11pt, a4paper, titlepage]{book}
\usepackage{graphicx} %package to manage images
\graphicspath{ {images/} }
% ! ACHTUNG !
% Nach dem ersten LaTeX durchlauf auf der Kommandozeile
% makeindex -s main.ist main.idx
% ausfŸhren, sonst wird der Index nicht so schšn formatiert.
% Leider macht TeXShop den makeindex Aufruf nur ohne Parameter.

% !ATTENTION!
% After running LaTeX for the first time using this template call
% makeindex -s main.ist main.idx
% to get the correct formatting for the index.

%Pakete und eigene Befehle - am besten durchlesen!
%Includes all neccessary packets and contains some useful commands - read this file!
%\include{i10preamble}
\usepackage{url}
\usepackage{listings}
\lstset{
	numbers=left,
	numberstyle=\tiny,
	numbersep=5pt,
	xleftmargin=11pt,
	xrightmargin=4pt,
	frame=single,
	aboveskip=0pt,
	belowskip=-6pt,
	sensitive=true,
	float=!t,
	breaklines=false,
	captionpos=b,
	tabsize=2,
	showstringspaces=false,
	basicstyle=\small\ttfamily,
	morecomment=[l]{//},
	morecomment=[s][\itshape]{/**}{*/}
}

\setlength{\voffset}{-1in}
\setlength{\hoffset}{-1in}

\setlength{\topmargin}{2.5cm}		   
\setlength{\headheight}{0cm}		   
\setlength{\headsep}{0cm}		   
\setlength{\oddsidemargin}{3.3cm}  % innen ein wenig mehr Rand für die Klebebindung
\setlength{\evensidemargin}{20.7cm} % dafür außen ein wenig weniger
\setlength{\textwidth}{15cm}		   
\setlength{\textheight}{23,5cm}		   
\setlength{\parindent}{0cm}
%--------------------------------------------------------------
%Dokumentspezifisches
%Stuff regarding your specific document
%--------------------------------------------------------------

%Trennungshilfen
%Hyphenation patterns
\hyphenation{
dieseswortwirdnichtgetrennt
diesesauchnicht
thiswordwillstayinoneline
thistoo
}

\usepackage{graphicx}

%--------------------------------------------------------------
\begin{document}

% grš§ere WortabstŠnde zulassen, um Trennungen zu vermeiden
% allow more flexible whitespaces to avoid hyphenation and overfull hboxes


% "see" EintrŠge fŸr den Index
% 'see'-entries for the index
\include{indexAddOn}


%--------------------------------------------------------------
\frontmatter

%Die Titelseite aus dem "images"-Verzeichnis wird verwendet
%use the titlepage from the 'images' directory
%Bachelor's: titlepage_Bachelor
%Master's:   titlepage_Master
%Neutral:    titlepage
% Dieses Teildokument beschreibt die Titelseite.
%

% Seitenzähler auf 1, Römische Ziffern.
\setcounter{page}{1}
\pagenumbering{roman}

\thispagestyle{headings}

% Eigentliche Titelseite.
\begin{titlepage}
	\begin{center}
		{\LARGE
			Rheinisch Westfälische Technische Hochschule Aachen \\
			Informatik 5 (Information Systems) \\
			\vspace{4em}
			
			\vspace{1em}
			
			\begin{figure}[h!]\begin{center}\resizebox{20em}{!}{\includegraphics{rwthlogo}}\end{center}\end{figure} 
			
			\vspace{1em}
			\begin{figure}[h!]\begin{center}\resizebox{10em}{!}{\includegraphics{i5-logo}}\end{center}\end{figure} 
			
			\vspace{1em}		
			
			{\LARGE \bf Master Thesis Proposal}\\ [1em]
			\bf Capturing Data Curation Process in Cancer Genomics \par} 
		
		\vspace{4em} 
		
		{\large \lineskip 1em
			
			\begin{tabular}[t]{rl}
				\multicolumn{2}{c}{\LARGE \bf Jasim Waheed Ansari}\\ 
				&\\
				{\Large \bf Aufgabenstellung:}&{\Large  Prof.\ Dr.\ Stefan \ Decker}\\[.5em]
				{\Large \bf Betreuer:}&{\Large Dr. Oya Deniz Beyan}\\
			\end{tabular}
			\par}
		
		\vspace{4em}
		{\Large	Aachen, den \today \par}
	\end{center}
\end{titlepage}

% \thispagestyle{empty}
% \emptydoublepage

% \include{declarationofownwork}
% \emptydoublepage

\setcounter{page}{5}

\tableofcontents
\cleardoublepage
%
% \listoffigures
% \emptydoublepage
%
% \listoftables
% \emptydoublepage

%Die †bersicht sollte in englischer und in deutscher Sprache verfasst werden
%the abstract should include an english and a german version
% \include{abstract}
% \emptydoublepage

%Danksagungen
%acknowledgements
% \include{acknowledgements}
% \emptydoublepage

%In der Arbeit verwendete Konventionen (Formelsatz, Definitionen, etc.)
%conventions applied in the thesis (AE/BE, definitions, etc.)
%\include{conventions}
%\emptydoublepage

%--------------------------------------------------------------
\mainmatter

\include{introduction}
\cleardoublepage
\include{relatedwork}
\cleardoublepage
\include{ownwork}
\cleardoublepage
\include{evaluation}
\cleardoublepage
\include{timeplan}
\cleardoublepage

% \begin{appendix}
% \include{appendix_01}
% \emptydoublepage
% \include{appendix_02}
% \emptydoublepage
% \end{appendix}

%--------------------------------------------------------------
\backmatter

%Bibliographie
%bibliography
\clearpage
%\phantomsection

\bibliographystyle{plain}
\bibliography{thesis}
\cleardoublepage

%Index
%index
\markboth{Index}{Index}
\thispagestyle{plain}
%\phantomsection

%\printindex

%Satzdatum
%date of typesetting


%--------------------------------------------------------------
\end{document}

